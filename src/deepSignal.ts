...
export const deepSignal = <T extends object>(obj: T): DeepSignal<T> => {
  ...
  if (!objToProxy.has(obj)) objToProxy.set(obj, createProxy(obj, objectHandlers) as DeepSignal<T>)
...
const get =
  ...
  (target: object, fullKey: string, receiver: object): unknown => {
    ...
    if (!isArrayOfSignals && returnSignal && Array.isArray(target)) {
      if (fullKey === '$') {
        if (!arrayToArrayOfSignals.has(target)) arrayToArrayOfSignals.set(target, createProxy(target, arrayHandlers))
...
const set =
  ...
  (target: object, fullKey: string, val: any, receiver: object): boolean => {
    ...
    if (fullKey[0] === '$') {
      ...
      else signals.get(fullKey)(internal)
      if (isNew && objToIterable.has(target)) {
        const iterable = objToIterable.get(target)
        iterable(iterable() + 1)
      }
...
const objectHandlers = {
  ...
  deleteProperty(target: object, key: string): boolean {
    ...
    if (signals && signals.has(key)) signals.get(key)(undefined)
